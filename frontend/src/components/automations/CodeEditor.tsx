...
import { Editor, createEditor, Descendant, NodeEntry, Range, Text, Transforms, Element as SlateElement } from 'slate';
...
export function CodeEditor({ value, onChange, language, onFullscreen }: CodeEditorProps) {
  ...
  // Convert value prop to Slate format - this is the controlled value
  const slateValue: Descendant[] = stringToSlateValue(value);

  // Get Prism language object
    ...
    return (
      <div {...attributes} className="flex">
        <span
          ...
          className="select-none text-muted-foreground/50 pr-3 flex-shrink-0 font-mono text-right"
          style={{ minWidth: '3ch' }}
        >
          {lineNumber}
        </span>
  ...
  // Placeholder text (minimal since we have boilerplate)

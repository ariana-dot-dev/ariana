...
impl FileSystemManager {
    ...
    pub fn copy_files_with_exclusion(
        ...
        // Always use the smart copy functions that handle both git and target exclusions
        match os_session {
            OsSession::Local(_) => {
    ...
    fn copy_files_local(source: &str, destination: &str, exclude_git: bool) -> Result<(), String> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if contains_cargo_projects {
                args.extend(vec!["/XD".to_string(), "target".to_string()]);
            }
        ...
        #[cfg(any(target_os = "linux", target_os = "macos"))]
        {
            ...
            if contains_cargo_projects {
                args.push("--exclude=target".to_string());
            }
            
            let source_with_star = format!("{}/*", source);
            args.push(source_with_star);
    ...
    fn copy_files_wsl(source: &str, destination: &str, distribution: &str, exclude_git: bool) -> Result<(), String> {
        ...
        if exclude_git || contains_cargo_projects {
            let mut rsync_args = vec!["-av"];
            if exclude_git {
                rsync_args.push("--exclude=.git");
            }
            if contains_cargo_projects {
                rsync_args.push("--exclude=target");
            }
            let source_with_star = format!("{}/*", source);
            rsync_args.push(&source_with_star);
            ...
            CommandExecutor::execute_with_os_session(
                ...
                let mut find_excludes = Vec::new();
                if exclude_git {
                    find_excludes.push("-name '.git' -prune -o");
                }
                if contains_cargo_projects {
                    find_excludes.push("-name 'target' -prune -o");
                }
                
                let find_cmd = if find_excludes.is_empty() {
                    format!("mkdir -p '{}' && cd '{}' && find . -type f -exec cp --parents {{}} '{}' \\;", destination, source, destination)
                } else {
                    format!("mkdir -p '{}' && cd '{}' && find . {} -type f -exec cp --parents {{}} '{}' \\;", 
                            destination, source, find_excludes.join(" "), destination)
                };
            ...
            CommandExecutor::execute_with_os_session(
                "cp", 
                &["-r", &source_with_star, destination], 
                None, 
                &OsSession::Wsl(wsl_session)
            ).map(|_| ())
        }

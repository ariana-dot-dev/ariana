...
struct AppInner {
    ...
    active_streamer: tokio::sync::Mutex<Option<tokio::process::Child>>,
}
...
impl App {
    pub async fn new(config: Config) -> Result<Self, anyhow::Error> {
        let app = AppInner {
            ...
            active_streamer: tokio::sync::Mutex::new(None),
        };
    ...
    /// Kill the currently active streamer process (if any) and store the new one.
    /// This ensures only one streamer runs at a time.
    pub async fn replace_active_streamer(&self, new_child: tokio::process::Child) {
        let mut guard = self.inner.active_streamer.lock().await;
        if let Some(mut old_child) = guard.take() {
            info!("[Stream] Killing previous streamer process before starting new one");
            if let Err(err) = old_child.kill().await {
                warn!("[Stream] Failed to kill previous streamer: {err}");
            }
        }

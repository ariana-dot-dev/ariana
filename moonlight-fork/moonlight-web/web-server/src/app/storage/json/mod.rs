...
impl JsonStorage {
    pub async fn load(
        ...
        // PERSISTENCE FIX: Restore admin session token if it exists
        let parent_dir = this.file.parent().unwrap_or(std::path::Path::new("."));
        let token_file = parent_dir.join("admin_session.token");
        if let Ok(token_str) = fs::read_to_string(&token_file).await {
            let token_str = token_str.trim();
            if let Ok(token) = SessionToken::decode(token_str) {
                // Check if admin user (id=1) exists
                let users = this.users.read().await;
                if !users.is_empty() {
                    // Get first user ID (assuming it's admin)
                    if let Some((&user_id, _)) = users.iter().next() {
                        drop(users); // Release read lock before acquiring write lock

                        let mut sessions = this.sessions.write().await;
                        // Restore session with 1 year expiration
                        sessions.insert(
                            token,
                            Session {
                                created_at: Instant::now(),
                                expiration: Duration::from_secs(365 * 24 * 3600),
                                user_id,
                            },
                        );
                        info!("Restored admin session token from file");
                    }
                }
            }
        }
...
impl Storage for JsonStorage {
    ...
    async fn create_session_token(
        ...
        // PERSISTENCE FIX: Check if this is the first session BEFORE inserting
        let is_first_session = sessions.is_empty();
        ...
        // Drop the write lock before doing file I/O
        drop(sessions);

        // Save the first session token to file (survives restarts)
        if is_first_session {
            let parent_dir = self.file.parent().unwrap_or(std::path::Path::new("."));
            let token_file = parent_dir.join("admin_session.token");
            let mut token_bytes = [0; 64];
            let token_str = token.encode(&mut token_bytes);
            if let Err(e) = std::fs::write(&token_file, token_str.as_bytes()) {
                warn!("Failed to persist admin session token: {}", e);
            } else {
                info!("Persisted admin session token (user_id={}) to {:?}", user_id.0, token_file);
            }
        }

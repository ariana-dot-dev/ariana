...
// All real-time input channels are unreliable + unordered (fire-and-forget, like video).
// If a packet is lost, the next one arrives soon anyway. Waiting for retransmits
// stalls input and causes the freeze-while-video-works bug.
// Only GENERAL (control signaling), MOUSE_RELIABLE (clicks), and CONTROLLERS (connect/disconnect) need delivery guarantees.
...
export const TRANSPORT_CHANNEL_OPTIONS: Record<keyof typeof TransportChannelId, TransportChannelOption> = {
    ...
    STATS: { reliable: false, ordered: false },
    HOST_VIDEO: { reliable: false, ordered: false },
    HOST_AUDIO: { reliable: false, ordered: false },
    MOUSE_RELIABLE: { reliable: true, ordered: true },
    MOUSE_ABSOLUTE: { reliable: false, ordered: false },
    MOUSE_RELATIVE: { reliable: false, ordered: false },
    KEYBOARD: { reliable: false, ordered: false },
    TOUCH: { reliable: false, ordered: false },

...
// Pure physical key mapping - sends physical key codes (event.code)
// User must set remote machine's keyboard layout to match their local layout
...
// Pure physical key mapping - event.code maps to VK code
// User must set remote machine's keyboard layout to match their local layout
export function convertToKey(event: KeyboardEvent): number | null {
    return VK_MAPPINGS[event.code] ?? VK_MAPPINGS[event.key] ?? null;
}

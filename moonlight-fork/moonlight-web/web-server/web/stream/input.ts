...
// Clipboard sync with remote desktop via xdotool-server
// Server runs on port 9091 on the streaming machine
let clipboardServerUrl: string | null = null;
let lastLocalClipboard: string = "";
let lastSyncTime: number = 0;
const SYNC_DEBOUNCE_MS = 100;

export function setClipboardServerUrl(url: string) {
    clipboardServerUrl = url;
    console.log(`[Clipboard] Server URL set to: ${url}`);

    // Setup focus listener to sync clipboard when window gains focus
...
async function onWindowFocus() {
    // When window gains focus, sync local clipboard to remote
    // This handles the case where user copied something outside the stream
    await syncClipboardToRemote();
}

async function syncClipboardToRemote(): Promise<boolean> {
    if (!clipboardServerUrl) return false;

    // Debounce rapid syncs
    const now = Date.now();
    if (now - lastSyncTime < SYNC_DEBOUNCE_MS) return false;
    lastSyncTime = now;

    // Clipboard API requires secure context (HTTPS) or localhost
    if (!navigator.clipboard?.readText) {
        console.debug('[Clipboard] Clipboard API not available (requires HTTPS)');
        return false;
    }
    try {
        const text = await navigator.clipboard.readText();
        if (!text || text === lastLocalClipboard) return false;

        lastLocalClipboard = text;
        const response = await fetch(`${clipboardServerUrl}/clipboard`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text })
        });
        console.log(`[Clipboard] Synced to remote: ${text.length} chars`);
        return response.ok;
    } catch (e) {
        // DOMException for clipboard access is expected without user gesture
        if (e instanceof DOMException && e.name === 'NotAllowedError') {
            console.debug('[Clipboard] Clipboard read requires user gesture');
        } else {
            console.warn('[Clipboard] Failed to sync to remote:', e);
        }
        return false;
    }
}

async function syncClipboardFromRemote(): Promise<boolean> {
    if (!clipboardServerUrl) return false;
    // Clipboard API requires secure context (HTTPS) or localhost
    ...
    try {
        ...
        if (data.text) {
            ...
            lastLocalClipboard = data.text; // Update local tracking to avoid re-sync
            console.log(`[Clipboard] Synced from remote: ${data.text.length} chars`);
            return true;
        }
        return false;
    } catch (e) {
        // DOMException for clipboard access is expected without user gesture
        if (e instanceof DOMException && e.name === 'NotAllowedError') {
            console.debug('[Clipboard] Clipboard write requires user gesture');
        } else {
            console.warn('[Clipboard] Failed to sync from remote:', e);
        }
        return false;
    }
}

// Metrics tracking for input latency benchmarking
...
(window as any).getInputMetrics = () => ({
    ...inputMetrics,
    avgLatencyMs: inputMetrics.keyDownCount > 0 ? inputMetrics.totalLatencyMs / inputMetrics.keyDownCount : 0,
    keysPerSecond: inputMetrics.keyDownCount / ((performance.now() - (window as any).__inputMetricsStart || 1) / 1000),
});
(window as any).__inputMetricsStart = performance.now();
(window as any).resetInputMetrics = () => {
    inputMetrics.keyDownCount = 0;
    inputMetrics.keyUpCount = 0;
    inputMetrics.totalLatencyMs = 0;
    inputMetrics.minLatencyMs = Infinity;
    inputMetrics.maxLatencyMs = 0;
    (window as any).__inputMetricsStart = performance.now();
};
...
// Encrypted send for WebRTC data channels (mouse movement)
// WebSocket channels handle encryption internally via WebSocketDataChannel
function trySendChannelEncrypted(channel: DataTransportChannel | null, buffer: ByteBuffer) {
    if (!channel) {
        return
    }
    ...
    encrypt(readBuffer).then((encrypted) => {
        channel.send(encrypted.buffer)
    }).catch((err) => {
        console.warn("[Input] Encrypt failed, dropping mouse packet:", err)
    })
}
...
export class StreamInput {
    ...
    // wsOverrides: channels that go over WebSocket (TCP) instead of WebRTC.
    // Keyboard/clicks/gamepad/touch → WebSocket (reliable, must arrive)
    // Mouse movement → WebRTC data channel (unreliable, low latency like video)
    setTransport(transport: Transport, wsOverrides?: Map<number, DataTransportChannel>) {
        const resolve = (id: TransportChannelIdValue): DataTransportChannel => {
            return wsOverrides?.get(id) ?? this.getDataChannel(transport, id)
        }

        this.keyboard = resolve(TransportChannelId.KEYBOARD)

        this.mouseReliable = resolve(TransportChannelId.MOUSE_RELIABLE)
        // Mouse movement uses WebRTC data channels (unreliable, low latency)
        ...
        this.touch = resolve(TransportChannelId.TOUCH)
        ...
        this.controllers = resolve(TransportChannelId.CONTROLLERS)
        ...
        for (let i = 0; i < 16; i++) {
            ...
            this.controllerInputs[i] = resolve(channelId)
        }
    }
    ...
    // Track pending clipboard sync for Ctrl+V so we can defer the keyup too
    private pendingCtrlVSync: Promise<void> | null = null;

    private sendKeyEvent(isDown: boolean, event: KeyboardEvent) {
        // Track metrics for benchmarking
        const now = performance.now();
        if (isDown) {
            inputMetrics.keyDownCount++;
            inputMetrics.lastKeyTime = now;
        } else {
            inputMetrics.keyUpCount++;
        }
        ...
        // Clipboard sync: Ctrl+V (paste) - sync local clipboard to remote before sending key
        if (isDown && event.ctrlKey && event.code === 'KeyV') {
            this.pendingCtrlVSync = syncClipboardToRemote().then(() => {
                this.doSendKeyEvent(isDown, key, event);
                this.pendingCtrlVSync = null;
            }, () => {
                // On error, still clear the pending state to avoid blocking future pastes
                this.pendingCtrlVSync = null;
            });
            return;
        }

        // If V keyup arrives while the Ctrl+V clipboard sync is still pending,
        // defer the keyup until after the keydown has been sent to avoid
        // dropping it (which would leave V stuck down, causing 'v' repeat loop)
        if (!isDown && event.code === 'KeyV' && this.pendingCtrlVSync) {
            this.pendingCtrlVSync.then(() => {
                this.doSendKeyEvent(isDown, key, event);
            });
            return;
        }

        // Clipboard sync: Ctrl+C (copy) - sync remote clipboard to local after key release
        if (!isDown && event.ctrlKey && event.code === 'KeyC') {
            this.doSendKeyEvent(isDown, key, event);
            // Small delay to let the remote app process the copy
            setTimeout(() => syncClipboardFromRemote(), 100);
            return;
        }
    ...
    private doSendKeyEvent(isDown: boolean, key: number, event: KeyboardEvent) {
        if (isDown) {
    ...
    sendKey(isDown: boolean, key: number, modifiers: number) {
        ...
        // Track send latency (time to queue in WebRTC buffer)
        const sendLatency = performance.now() - sendStart;
        if (isDown) {
            inputMetrics.totalLatencyMs += sendLatency;
            inputMetrics.minLatencyMs = Math.min(inputMetrics.minLatencyMs, sendLatency);
            inputMetrics.maxLatencyMs = Math.max(inputMetrics.maxLatencyMs, sendLatency);
        }
    }
    ...
    sendMouseMove(movementX: number, movementY: number) {
        ...
        // Mouse movement goes over WebRTC (unreliable, low latency) - encrypted
        trySendChannelEncrypted(this.mouseRelative, this.buffer)
    }
    ...
    sendMousePosition(x: number, y: number, referenceWidth: number, referenceHeight: number, reliable: boolean) {
        ...
        if (reliable) {
            // Reliable mouse position goes over WebSocket (via wsOverrides)
            ...
            // Mouse position goes over WebRTC (unreliable, low latency), encrypted
            trySendChannelEncrypted(this.mouseAbsolute, this.buffer)
        }
    }
    ...
    sendMouseWheelHighRes(deltaX: number, deltaY: number) {
        ...
        // Wheel on WebRTC (mouseRelative), encrypted
        trySendChannelEncrypted(this.mouseRelative, this.buffer)
    }

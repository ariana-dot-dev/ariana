...
export async function getApi(): Promise<Api> {
    ...
    // URL Token Auth - check for ?token= parameter and set cookie
    const urlParams = new URLSearchParams(window.location.search);
    const urlToken = urlParams.get('token');
    if (urlToken) {
        document.cookie = `mlSession=${urlToken}; path=/; SameSite=None; Secure`;
        console.log('[API] Set session cookie from URL token');
    }

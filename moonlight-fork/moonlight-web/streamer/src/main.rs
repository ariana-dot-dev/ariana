...
struct StreamConnection {
    ...
    pub transport_sender: RwLock<Option<Box<dyn TransportSender + Send + Sync + 'static>>>,
...
impl StreamConnection {
    pub async fn new(
        ...
        let this = Arc::new(Self {
            ...
            transport_sender: RwLock::new(None),
            ...
            async move {
                while let Some(message) = ipc_receiver.recv().await {
                    ...
                    if let ServerIpcMessage::Stop = &message {
                        info!("[IPC] Received Stop from server, terminating streamer");
                        this.stop().await;
                        return;
                    }
    ...
    async fn set_transport(
        ...
        let old_transport = {
            let mut sender = this.transport_sender.write().await;
    ...
    async fn try_send_packet(&self, packet: OutboundPacket, packet_ty: &str, should_warn: bool) {
        let sender = self.transport_sender.read().await;
    ...
    async fn on_ipc_message(self: &Arc<StreamConnection>, message: ServerIpcMessage) {
        if let ServerIpcMessage::WebSocket(StreamClientMessage::SetTransport(_transport_type)) =
            &message
        {
            info!("Starting WebRTC transport (unified, no alternatives)");
    ...
    async fn stop(&self) {
        ...
        let mut transport = self.transport_sender.write().await;

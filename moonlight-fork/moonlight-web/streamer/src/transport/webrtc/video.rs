...
pub struct WebRtcVideo {
    ...
    /// Timestamp (ms since peer creation) when setup completed, for first-frame delta
    setup_complete_ms: AtomicU64,
}
...
impl WebRtcVideo {
    pub fn new(runtime: Handle, peer: Weak<RTCPeerConnection>, frame_queue_size: usize) -> Self {
        Self {
            ...
            setup_complete_ms: AtomicU64::new(0),
        }
    }
    ...
    pub async fn setup(
        ...
        info!("[VIDEO-SETUP] Parameters: {width}x{height}x{redraw_rate} format={format:?}");
        info!("[VIDEO-SETUP] Supported formats: {:?}", self.supported_video_formats);
        info!("[VIDEO-SETUP] Peer signaling state: {:?}", inner.peer.signaling_state());
        info!("[VIDEO-SETUP] Peer ICE connection state: {:?}", inner.peer.ice_connection_state());
        info!("[VIDEO-SETUP] Peer connection state: {:?}", inner.peer.connection_state());
        ...
        if !format.contained_in(self.supported_video_formats) {
            ...
            error!("[VIDEO-SETUP] FORMAT NOT SUPPORTED: {}", message);
            ...
            if let Err(err) = inner
                ...
                .await
            {
                warn!("[VIDEO-SETUP] Failed to send error to client: {err}");
            }

            return false;
        }

        info!("[VIDEO-SETUP] Format is supported, getting codec...");
        let Some(codec) = video_format_to_codec(format) else {
            error!("[VIDEO-SETUP] FAILED to get video codec for format {:?}", format);
            return false;
        };
        info!("[VIDEO-SETUP] Got codec: {:?}", codec.capability);

        info!("[VIDEO-SETUP] Creating video track...");
            ...
            .await
        {
            ...
            error!("[VIDEO-SETUP] TRACK CREATION FAILED: {}", message);
        ...
        info!("[VIDEO-SETUP {}] Track created successfully", inner.t_plus());
        info!("[VIDEO-SETUP] Peer signaling state AFTER track creation: {:?}", inner.peer.signaling_state());
        info!("[VIDEO-SETUP] Peer ICE connection state AFTER track creation: {:?}", inner.peer.ice_connection_state());

        self.clock_rate = codec.capability.clock_rate;
        info!("[VIDEO-SETUP] Clock rate set to: {}", self.clock_rate);

        self.codec = match format {
            // -- H264
            VideoFormat::H264 | VideoFormat::H264High8_444 => {
                info!("[VIDEO-SETUP] Setting up H264 codec");
            ...
            | VideoFormat::H265Rext10_444 => {
                info!("[VIDEO-SETUP] Setting up H265 codec");
            ...
            | VideoFormat::Av1High10_444 => {
                info!("[VIDEO-SETUP] Setting up AV1 codec");
                Some(VideoCodec::Av1 {
        ...
        // Renegotiate to inform browser about the new video track
        // CRITICAL: This must complete in < 10 seconds or Sunshine will disconnect!
        info!("[VIDEO-SETUP {}] ========== STARTING RENEGOTIATION ==========", inner.t_plus());
        info!("[VIDEO-SETUP] CRITICAL: Renegotiation must complete in < 10 seconds (Sunshine timeout)");
        info!("[VIDEO-SETUP] About to call inner.send_offer()");
        info!("[VIDEO-SETUP] Peer signaling state BEFORE renegotiation: {:?}", inner.peer.signaling_state());
        info!("[VIDEO-SETUP] Peer ICE connection state BEFORE renegotiation: {:?}", inner.peer.ice_connection_state());
        info!("[VIDEO-SETUP] Peer ICE gathering state BEFORE renegotiation: {:?}", inner.peer.ice_gathering_state());

        let start_time = std::time::Instant::now();
        let renegotiation_result = inner.send_offer().await;
        let elapsed = start_time.elapsed();
        info!("[VIDEO-SETUP] Renegotiation took {:?}", elapsed);

        info!("[VIDEO-SETUP] send_offer() returned: {}", renegotiation_result);
        info!("[VIDEO-SETUP] Peer signaling state AFTER renegotiation: {:?}", inner.peer.signaling_state());
        info!("[VIDEO-SETUP] Peer ICE connection state AFTER renegotiation: {:?}", inner.peer.ice_connection_state());
        info!("[VIDEO-SETUP] Peer ICE gathering state AFTER renegotiation: {:?}", inner.peer.ice_gathering_state());
        info!("[VIDEO-SETUP] Peer connection state AFTER renegotiation: {:?}", inner.peer.connection_state());
        ...
        self.setup_complete_ms.store(inner.created_at.elapsed().as_millis() as u64, Ordering::Relaxed);
    ...
    pub async fn send_decode_unit(&mut self, unit: &VideoDecodeUnit<'_>) -> DecodeResult {
        if !self.first_frame_sent {
            ...
            let setup_ms = self.setup_complete_ms.load(Ordering::Relaxed);
            info!("[VIDEO] First video frame being sent (setup completed at T+{}ms)", setup_ms);
        }

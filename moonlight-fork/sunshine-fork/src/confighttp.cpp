...
namespace confighttp {
  ...
  /**
   * @brief Change video bitrate dynamically during streaming.
   * @param response The HTTP response object.
   * @param request The HTTP request object.
   *
   * @api_examples{/api/bitrate| POST| {"bitrate":5000}}
   */
  void setBitrate(resp_https_t response, req_https_t request) {
    ...
    try {
      input_tree = nlohmann::json::parse(request->content);
    } catch (const std::exception &e) {
      BOOST_LOG(error) << "Failed to parse JSON: " << e.what();
      output_tree["status"] = false;
      output_tree["error"] = "Invalid JSON";
      send_response(response, output_tree);
      return;
    }

    if (!input_tree.contains("bitrate")) {
      output_tree["status"] = false;
      output_tree["error"] = "Missing 'bitrate' field";
      send_response(response, output_tree);
      return;
    }

    int bitrate_kbps = input_tree["bitrate"].get<int>();
    if (bitrate_kbps < 500 || bitrate_kbps > 150000) {
      output_tree["status"] = false;
      output_tree["error"] = "Bitrate must be between 500 and 150000 kbps";
      send_response(response, output_tree);
      return;
    }

    BOOST_LOG(info) << "API request to change bitrate to " << bitrate_kbps << " kbps";
    video::set_pending_bitrate(bitrate_kbps);

    output_tree["status"] = true;
    output_tree["bitrate"] = bitrate_kbps;
    send_response(response, output_tree);
  }
  ...
  void start() {
    ...
    server.resource["^/api/bitrate$"]["POST"] = setBitrate;

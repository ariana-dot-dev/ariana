...
namespace video {
  ...
  // Atomic variable for dynamic bitrate changes
  static std::atomic<int> pending_bitrate_kbps{0};

  void set_pending_bitrate(int bitrate_kbps) {
    BOOST_LOG(info) << "Setting pending bitrate change to " << bitrate_kbps << " kbps";
    pending_bitrate_kbps.store(bitrate_kbps, std::memory_order_release);
  }

  int get_and_clear_pending_bitrate() {
    return pending_bitrate_kbps.exchange(0, std::memory_order_acq_rel);
  }
  ...
  // Forward declaration for dynamic bitrate changes
  ...
  void encode_run(
    ...
    // Track current bitrate for dynamic changes
    int current_bitrate = config.bitrate;
    ...
    while (true) {
      ...
      // Check for dynamic bitrate change
      int new_bitrate = get_and_clear_pending_bitrate();
      if (new_bitrate > 0 && new_bitrate != current_bitrate) {
        BOOST_LOG(info) << "Dynamic bitrate change: " << current_bitrate << " -> " << new_bitrate << " kbps";

        // Update config with new bitrate
        config.bitrate = new_bitrate;
        current_bitrate = new_bitrate;

        // Create new encode device
        auto new_encode_device = make_encode_device(*disp, encoder, config);
        if (!new_encode_device) {
          BOOST_LOG(error) << "Failed to create new encode device for bitrate change";
        } else {
          // Teardown old session asynchronously if supported
          if (encoder.flags & ASYNC_TEARDOWN) {
            std::thread encoder_teardown_thread {[old_session = std::move(session)]() mutable {
              BOOST_LOG(info) << "Starting async encoder teardown for bitrate change";
              old_session.reset();
              BOOST_LOG(info) << "Async encoder teardown complete";
            }};
            encoder_teardown_thread.detach();
          ...
          // Create new session with updated bitrate
          session = make_encode_session(disp.get(), encoder, config, disp->width, disp->height, std::move(new_encode_device));
          if (!session) {
            BOOST_LOG(error) << "Failed to create new encode session for bitrate change";
            return;
          }

          // Load dummy image into new session
          auto dummy_img = disp->alloc_img();
          if (!dummy_img || disp->dummy_img(dummy_img.get()) || session->convert(*dummy_img)) {
            BOOST_LOG(error) << "Failed to initialize new session after bitrate change";
            return;
          }

          BOOST_LOG(info) << "Successfully changed bitrate to " << new_bitrate << " kbps";

          // Request IDR frame for clean start at new bitrate
